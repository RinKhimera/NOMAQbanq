...
const getRole = (session: ActiveSession) => session.user.role as AppRole | undefined;
...
export const requirePermission = async (permissions: PermissionInput): Promise<ActiveSession> => {
  const session = await requireSession();

...
const requiredUrl = (label: string) => z.url({ error: `${label} : URL invalide ou manquante` });

/** Prod-like = prod/preview Vercel, sinon NODE_ENV. */
export const isProdLike = (s: Record<string, string | undefined> = process.env): boolean =>
...
export const loadServerEnv = (
  ...
  const result = buildServerSchema(isProdLike(source)).safeParse(stripEmpty(source));

...
export const updatePost = async (
  ...
  try {
    await db.update(posts).set({ title, body, updatedAt: new Date() }).where(eq(posts.id, id)); // ADAPT: + vérifier l'ownership (posts.authorId === session.user.id)
  } catch (error) {
    if (process.env.NODE_ENV !== 'production') console.error('[updatePost] DB error:', error);

...
- **Tentative de réfutation (concluante)** : Stripe **n'émet jamais** `checkout.session.expired`
  ...
  _exclusif-ou_ `open → expired` (après 24 h sans paiement). Les deux événements ne
...
1. **Double-crédit sur livraisons concurrentes du même event** — _disculpé_.
...
2. **Cumul perdu (lost-update) sous concurrence** — _disculpé_. Deux sessions payées
...
3. **Webhook spoofable** — _disculpé_. `route.ts:39-51` vérifie la signature sur le corps
...
4. **Open-redirect via `successPath`/`cancelPath`/`returnPath`** — _disculpé_. `safePath`
...
5. **`not_found` répondu 200 → crédit perdu sur course webhook-avant-insert** — _disculpé_.
...
6. **`productCode` falsifié pour payer moins cher** — _disculpé_. `actions.ts:320-340`
...
7. **`amount_total` non revérifié au webhook → sous-paiement** — _disculpé_. Le montant est
...
8. **Combo écrase un accès existant plus long** — _disculpé_. Combo calcule
...
9. **IDOR sur `verify`** — _disculpé_ (durcissement qui tient). `actions.ts:418-420` refuse

...
**Révision** : revue adversariale 2026-06-28 — correctifs intégrés (garde
`getExamLeaderboard` pour examens restreints ; `finalizeExam` tolérant au retrait
d'audience en cours)
...
- Examen **`subscribers`** : re-vérifier `hasAccess("exam")` (parité avec le
  re-check `userAccess` actuel — empêche de soumettre après expiration).
- Examen **`restricted`** : **NE PAS** re-vérifier l'appartenance à `examAudience`.
...
### Leaderboard & autres lectures d'examen restreint (⚠️ revue 🟠 #3)

`getExamLeaderboard` ([`features/exams/dal.ts:959`](../../../features/exams/dal.ts#L959))
est aujourd'hui accessible à tout abonné examen ayant participé **ou** à accès
actif → un non-membre verrait le classement d'un examen **restreint**
confidentiel. Le garde d'audience s'applique donc aux **quatre** points :
`getExamsWithParticipation` (liste), `startExam`, `finalizeExam`, **et
`getExamLeaderboard`** — ainsi qu'à toute autre lecture exposant un examen
restreint (page détail étudiant, page résultats). Règle commune : pour un examen
`restricted`, exiger l'appartenance à `examAudience` (ou admin) **avant** de
renvoyer la moindre donnée de l'examen.
...
- **Intégration** (`tests/integration/`, branche Neon éphémère) :
  ...
  - **Leaderboard** : examen restreint masqué aux non-membres, visible membres +
    admin (constat #3).
  - **`finalizeExam` tolérant** : un utilisateur retiré de l'audience **après**
    avoir démarré peut quand même finaliser (constat #6).
...
- **Cohérence des gardes d'audience** : la sémantique « sélection = accès » doit
  être appliquée à **tous** les points qui exposent un examen restreint — liste,
  `startExam`, `getExamLeaderboard`, pages détail/résultats — sinon fuite de
  visibilité ou d'accès. (`finalizeExam` est volontairement tolérant pour le
  restreint, cf. #6.)
